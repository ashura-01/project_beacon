...
class _VictimScreenState extends State<VictimScreen> {
  ...
  void _listenToSOS() {
    _dbRef.child("sos_alerts").onChildAdded.listen((event) {
      if (event.snapshot.value != null) {
        ...
        data['key'] = event.snapshot.key; // store the push key
        setState(() {
  ...
  Future<void> _reloadAlerts() async {
    ...
    if (snapshot.exists) {
      ...
      (snapshot.value as Map).forEach((key, value) {
        ...
        map['key'] = key; // store the push key
        newAlerts.add(map);
      });
  ...
  void _deleteAlert(Map<String, dynamic> alert) async {
    final key = alert['key'];
    if (key != null) {
      await _dbRef.child("sos_alerts/$key").remove();
      setState(() {
        alerts.remove(alert);
      });
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("SOS alert deleted")),
      );
    }
  }
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                Wrap(
                                  spacing: 12,
                                  runSpacing: 4,
                                  children: emergencyContacts.map((num) {
                                    return ElevatedButton.icon(
                                      onPressed: () => _copyNumber(num),
                                      icon: const Icon(
                                        Icons.copy,
                                        size: 16,
                      ...
                      trailing: SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            ...
                            if (alertUid == currentUserId)
                              IconButton(
                                icon: const Icon(Icons.delete, color: Colors.red),
                                onPressed: () => _deleteAlert(alert),
                                tooltip: "Delete your SOS alert",
                              ),
                          ],
                        ),
                      ),
